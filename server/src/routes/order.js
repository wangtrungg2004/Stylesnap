...
function buildPaymentSection({ method, bank, qrProofUrl }) {
  ...
  const proofHtml = qrProofUrl ? `
    ...
    </div>` : `<div style="color:#666">(Chưa kèm ảnh xác nhận)</div>`;
...
router.post("/", async (req, res) => {
  try {
    const {
      name, phone, email, address, method,
      // thiết kế
    ...
    const paymentBlock = buildPaymentSection({ method, bank: safeBank, qrProofUrl: safeProofUrl });
